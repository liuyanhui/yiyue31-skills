...
- **金句提取**：筛选出彩表达、亮眼、有记忆点的句子，作为总结中的亮点。金句必须是**完整句子**。
...
2. **逐个话题深入讨论**（使用自由文本对话，不使用预设选项）：
   ...
   - 用户可随时说"够了"、"继续下一个"、"跳过"来控制节奏
3. **退出讨论前必须用户确认**
   - AI **不得自行跳过**讨论环节
   - 退出前，AI 必须明确询问："讨论到此结束，可以继续下一步吗？"
   - 用户确认后才进入 Step 5

**讨论过程中 AI 的职责：**

- 记录用户原话，保持用户表达的完整性和真实性
- 提炼用户表达的观点、偏好、侧重点（供 Step 5 推荐风格时参考）
- 不做"AI问AI"自问自答——改为"AI分享理解 → 邀请用户评价/补充"
- 每次只讨论一个话题，聊透再换下一个
- 没有被选中的话题，不主动发起讨论
...
**输入类型判断：**
根据 `templates/input-types.md` 判断输入类型：
...
**选取原则：**
- 优先选择有感染力、有记忆点的金句
- 金句和习语必须是**完整句子**，不允许截取半句话
- 术语选择核心概念，避免过多
- 保持文章流畅性，不要过度打断正文

**保存位置：**
```
{title}/takeaways/takeaways-{title}.md
```
...
4. 文章概述是否连贯自然
5. 是否有逻辑问题或表达不清之处
6. 是否保持了原文的核心观点和用户的理解
7. 读者是否容易理解
8. 审查结果保存到 `{title}/takeaways/final-gan-evaluation-{title}.md` 中
