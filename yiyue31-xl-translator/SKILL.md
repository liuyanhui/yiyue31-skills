...
- **C4 派发口径模板**：审校派发提示词**必须**包含以下四项，缺一即口径事故风险：
...
- **C5 跨维采纳回核规则**：凡采纳非 accuracy 维（翻译腔/可读性/AI 味）的**结构性改写**（化定语为主谓、拆句重组、语序重排），改后必须对照原文回核**数字、比例、否定极性**——如"the share of X"被改写成"有多少 X"即丢比例义；词级替换（换量词/换搭配）不触发此回核。
...
- **状态命令 = resume oracle**（`status.mjs`）：冷启动入口；同文件再触发自动检测进行中并续跑；**输出双段契约**——人话一行 + 动作指令。**C3 view/dispatch 拆分**：无动词或动词=`progress` 默认**只读渲染**（零副作用——不物化 staging、不追加事件）；动词=`dispatch` 时才物化 + 追加事件——"运行一次 = 派发一轮承诺"不再是默认行为。状态纯推导自文件系统，**唯一续跑文档 `status.md`**。
- **D1 环境处置模板**：API 限流/流停滞的编排侧处置：
...
- **会话预算**：每会话处理 N 单元后干净退出；每单元幂等；N 同时约束内存与主会话上下文增长；**单元 = 一次 subagent 调用**（基线 ~11 单元/chunk）。
