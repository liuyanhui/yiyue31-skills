...
1. We can start a new, empty conversation without the accumulated context.
   ...
   It might still be a good idea to do, because the performance of LLM outputs decrease as the context size grows.
...
The number of retained messages varies because Pi uses a configurable token budget.
...
2. The user message in the compaction request is also different.
   It requests "a structured summary of this conversation branch for context when returning later."
...
This keeps the compacted context readable and portable, since we can switch models in Pi and continue using the summary.

## Compaction and prompt caching

Prompt caching is used by LLM providers to make repeated requests in the same conversation less expensive.
