...
- **生成总结后统计字数**：使用`scripts/word-counter.js`统计生成的总结文件字数，确保符合模板长度要求。

**字数统计示例：**

```bash
# 统计生成的总结文件字数
node scripts/word-counter.js {title}/summary-{title}.md
```

**预期输出示例：**

```
📄 File: {title}/summary-{title}.md
...
**字数检查标准：**
- **Standard模板**：300-500词或800-1500字
- **Concise模板**：根据模板要求检查
- **Comprehensive模板**：根据模板要求检查
- 如果字数不符合要求，返回步骤2重新生成或调整内容长度
