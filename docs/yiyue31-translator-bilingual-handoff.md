...
> **状态**：设计定稿，用户已全部拍板（2026-09-18），**实施未开始**；三份文档已提交推送远端（换机+冷启动事实源 = origin/master）。
...
| 2 | 三份文档 commit + push | 本轮提交 | ✅ 完成 |
| 3 | yiyue31-xl-translator 实现双语输出 | Phase 1（§2） | ⬜ 待开工 |
| 4 | yiyue31-translator 移除大文档（多 chunk）功能 | Phase 2（§3） | ⬜ 待开工 |
| 5 | yiyue31-translator 双语输出 | Phase 2 T2.4 | ⬜ 待开工 |
| 6 | 实施完成后开发期文档清理（用户逐字存档永久保留） | §5 | ⬜ 待实施 |
| 7 | 多角色 review 本次需求（是否启用） | §4 末尾建议 | ⬜ 待拍板 |
...
## 2. Phase 1：xl-translator 双语输出（先做）
...
## 3. Phase 2：yiyue31-translator 瘦身 + 双语
...
- 提交信息按仓库现行风格（中文、类型前缀、说清 why）；Phase 1、Phase 2 各自独立提交。

**多角色 review 建议（2026-09-18，待用户拍板是否启用）**：启用则三角色并行单轮、只审文档不改代码——① 冷启动读者：只读 HANDOFF+仓库模拟新会话开工，暴露自足性缺口；② xl 内核审查：对照 xl SKILL.md 逐机制核对 §1 决策有无冲突（红线③/终检重执行哲学/状态机动词/brief 语义/交付后纪律）；③ 删除面审计：grep translator skill 全部文件对 doc_segmenter、chunks、manifest、consistency-checklist、多 chunk 的引用，核对 Phase 2 清单完备性。产出问题清单 → 修订 HANDOFF → 再开工。不建议更多角色：文档审查三个聚焦视角已覆盖风险面。
