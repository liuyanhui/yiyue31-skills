...
8. **Standout section**: render `## 意外之声` (en: `## Surprising takes`; all skeletons, before 总结) from `02-grouped.json` `standouts`. Source and surprise bar are defined in Step 6.4. Render each pick as ONE blockquote with four fields separated by BLANK lines (so they survive HTML conversion as separate lines): `> **作者 @{author}**` / blank / `{中文译文}` / blank / `原文：*{original exact words}*` / blank / `为何意外：{reason}` (en relabels to `Author` / `Why surprising`). Join `@{author}` and the original exact words from `01-raw-data.json` by `commentId`; translate the original when lang differs (otherwise show the quote once). Attribution is allowed here — the section is an exact-quote spotlight. Omit the section entirely when `standouts` is empty; do not emit an empty heading.
