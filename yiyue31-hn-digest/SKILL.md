...
8. **Standout section**: render `## 意外之声 / Standout takes` (all skeletons, before 总结) from `02-grouped.json` `standouts`. Each pick is a blockquote of the comment's **exact source-language words** (do NOT translate the quote — a zh article quotes the English original), followed by three labeled lines: 作者/Author (`author`), 翻译/Translation (`translation`; omit when source language = `config.lang`), 入选原因/Reason for inclusion (`reason`, one line). Labels follow `config.lang`. Omit the section entirely when `standouts` is empty. Full format in `assets/article-{templateVersion}.md`.
