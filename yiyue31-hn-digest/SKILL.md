...
2. **Language**: Entire article in `config.lang`. Section headings are MONOLINGUAL (follow `config.lang`, never bilingual "zh / en"). H1 must be `[HN] {title in config.lang}` — for zh articles that is the CHINESE translation of `post.title` (HN titles are usually English); for en, `post.title` as-is. When `post.title` differs from the article language, add one line right after the H1: zh `<small>原标题：{post.title}</small>`, en `<small>Original: {post.title}</small>` (the injected header sits between H1 and this line). Skip it when `post.title` is already in the article language.
...
7. **Coverage (injected header — no end note, no per-section markers)**: do NOT append `（N / M 条）` / `(N / M comments)` to section headings — that ratio is an internal coverage metric; readers cannot interpret it and it clashes with the editorial voice. Do NOT write any coverage/methodology note at the end of the article. The disclaimer + methodology/neutrality + discussion snapshot (timestamp, post score, comment count) are injected as ONE `<small>` paragraph after the H1 by Step 10.3's `insert-header.ts`; the body ends after `## 参考资料`. Full rules in `assets/article-{templateVersion}.md`.
8. **Standout section**: render `## 意外之声` (en: `## Surprising takes`; all skeletons, before 总结) from `02-grouped.json` `standouts`. Source and surprise bar are defined in Step 6.4 — picks come from the outlier pool and must NOT repeat the body. Each pick is ONE blockquote: the comment's **exact source-language words** (do NOT translate the quote — a zh article quotes the English original), then three labeled lines 作者/Author (`author`), 翻译/Translation (`translation`; omit when source language = `config.lang`), 入选原因/Reason for inclusion (`reason`, one line). Put a BLANK `>` line between EVERY field so each renders as its own paragraph in HTML (consecutive `>` lines collapse into one paragraph — the line-break bug to avoid). Labels follow `config.lang`. Omit the section entirely when `standouts` is empty; do not emit an empty heading. Full format in `assets/article-{templateVersion}.md`.
...
10. **Formulas & math**: render any formula with LaTeX — display math as `$$...$$` on its own line, inline math as `$...$`. Never write formulas as plain prose (subscripts/superscripts get lost: write `$S_t = S_{t-1} + \beta_t(v_t - S_{t-1}k_t)k_t^T$`, not `St = St−1 + ...`). When the article is converted to HTML, the publisher must include a MathJax/KaTeX script for the LaTeX to render. Detailed constraint in `assets/article-{templateVersion}.md`.
