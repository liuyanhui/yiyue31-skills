...
- Each pick must render as ONE blockquote with four fields separated by BLANK lines: `**作者 @{author}**`, the Chinese translation, `原文：*{original exact words}*`, and `为何意外：{reason}` (en relabels to `Author` / `Why surprising`). Blank-line separation is required so the fields survive HTML conversion as separate lines instead of collapsing onto one.
...
- Defects: section missing despite non-empty standouts; empty heading when standouts is empty; fields collapsed onto one line (missing blank lines between them); picks that repeat the body; picks that are not actually surprising; blockquotes that paraphrase or fabricate instead of quoting exact words.
