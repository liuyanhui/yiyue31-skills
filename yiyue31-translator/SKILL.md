...
4. 当不确定topic是否正确时，使用 AskUserQuestion 工具列出候选 topic 请用户选择或输入自定义意见。
5. 原文是超链接时，提取链接
6. 分析结果保存到本地，目录为：`{title}/translation/analysis-topic-{title}.md`。

**topic识别规则**：优先从标题、h2/h3中提取；识别技术关键词；多topic选择主要topic；未知topic询问用户。
**语言识别**：如果输入内容主要是中文（如：中文超过50%），使用 AskUserQuestion 工具请用户确认是否继续；如果主要语言是非英文的其他语言，使用 AskUserQuestion 工具请用户确认是否继续。
...
使用 AskUserQuestion 工具请用户确定翻译风格，或输入自定义意见：
| 选项 | 说明 |
|------|------|
...
3. **展示新术语**：展示新术语列表
4. **用户确认**：使用 AskUserQuestion 工具，请用户确认：全部添加、部分添加、跳过或用户输入意见。
...
- **特殊词句翻译规则**：对于金句、'-'连接的词组、俚语和习语，按照[特殊词句提取](#step-6-特殊词句提取)中提取结果文件中的`中文翻译（English original）`列进行翻译。
- **金句、俚语和习语**：加粗展示，如:`**{金句}**`。
- **原文链接**：对于原文中的链接，翻译时保留链接地址不变，但可以翻译链接文本（如果有）。例如：`[原文](https://example.com)` 翻译为 `[示翻译](https://example.com)`。
