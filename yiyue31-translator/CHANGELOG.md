...
真任务验证（abc-legal-builder-agents，4 条发现见 `~/project/translator-v301-test/RESULTS.md`）后用户按建议拍板：

- **frontmatter 处置口径入 SKILL（发现 1）**：源含 YAML 剪藏元数据 → 元信息以引用块进译文草稿顶部（供 Step 10 头取材 + URL 子集校验自然通过）——本次实测"不进正文"会撞机械校验误报。
...
### SKILL 提示词精简（用户拍板，版本号不变）

- **Step 8 与 Corrections 双源合并**：收录/移除标准单一来源归 Corrections 节（新增移除标准行），Step 8 改为引用。
- **路径约定单行表压缩为一行**（一条数据行的表无信息增量）。
- **三处措辞收紧**：功能描述去空话；修辞处理并入"意译"定义（该规则本就只在意译下成立）；Step 4 临时标记的清理细节归属 Step 10，仅留一句指引。
- **xl-translator 同批**：删 5 处 M3 事故出处注记（motive-why，规则与 C5 具体反例保留）。
